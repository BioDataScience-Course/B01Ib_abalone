...
<!--# Introduction et but ont été réduits à leur plus simple expression, mais normalement cette section est plus détaillée et étayée de références bibliographiques (ce n'est pas important pour l'exercice ici) ! -->
...
<!--% Utilisez le chunk ci-dessous pour transformer les données, et en refaire un nouveau graphique. -->
...
<!--% Faites un résumé de votre modèle avec la fonction `summary_()` (ou summary()) appliquée sur votre objet `abalone_lm` et affichez-le grâce à `tabularise()`.-->
...
<!--% Étudiez l’influence des individus sur la régression linéaire grâce à l'effet de levier (axe X) avec les résidus standardisés sur l'axe Y et avec la distance de Cook (taille des points). -->
...
[] -   Une observation influente est observée, mais la masse de données et leur répartition permettent de considérer que le modèle reste fiable malgré tout.
[] -   Plusieurs observations très influentes sont observées et la validité du modèle est mise en cause.
[] -   Plusieurs observations influentes sont observées, mais la masse de données et leur répartition permettent de considérer que le modèle reste fiable malgré tout.
...
<!--# Attention, la corrélation n'est pas un critère absolu pour déterminer les meilleures variables pour une régression, car bien d'autres critères peuvent entrer en compte. D'autre part, souvenez-vous du jeu de données 'anscombe' qui démontrait que l'on peut avoir une même corrélation pour des formes très différentes du nuage de points ! C'est toutefois un guide utile comme première approche. -->

<!--% Normalement, vous devriez étudier les graphiques et les transformations éventuelles des données plus à fond avant de choisir les variables de votre modèle, mais cela dépasserait le temps imparti pour cet exercice. Faites donc directement une régression de log_weight par rapport à l'âge dans la variable `abalone_lm2` et directement dans la foulée le graphique de la régression avec `chart()`. -->
...
[] -   Les paramètres sont estimés avec une faible précision, leurs écarts types sont importants.
[] -   Les paramètres sont bien estimés, leurs écarts types sont proportionnellement raisonnables.
[] -  Les estimateurs des paramètres sont précis, les écarts types sont faibles.
...
[] -   Une observation influente est observée, mais la masse de données et leur répartition permettent de considérer que le modèle reste fiable malgré tout (D).
