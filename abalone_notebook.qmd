...
La régression linéaire simple par les moindres carrés est utilisée pour modéliser les données. L'analyse est réalisée avec la [SciViews Box 2023](https://www.sciviews.org/software/svbox/) dans [Saturn Cloud](https://saturncloud.io) (Linux) avec le [logiciel R](https://www.r-project.org) (`r R.version.string`).
...
```{r correlation1, record='RNP', arg='col_keys'}
abalone %>.%
  sselect(___) %>.%
  ___ %>.%
  ___
...
```{r chart1, record='RNSTR', arg='mapping,layers'}
___
```
...
```{r chart2, record='RNSTR', arg='mapping,layers,labels'}
abalone %>.%
  smutate(.,
...
```{r lm1summary, record='RNP', arg='col_keys'}
___
```
...
```{r resid1, record='RNP', arg='labels'}
___
```
...
```{r resid2, record='RNP', arg='labels'}
___
```
...
```{r resid3, record='RNP', arg='labels'}
___
```
...
```{r resid4, record='RNP', arg='labels'}
___
```
...
### Modélisation de la masse en fonction de l'âge

Nous cherchons maintenant à prédire l'âge des ormeaux à l'aide d'une variable morphométrique par régression linéaire simple. Pour choisir la meilleure variable, nous analysons les corrélations comme première approche.

```{r correlation2, record='RNP', arg='col_keys'}
abalone %>.%
  sselect(., ___) %>.%
  ___ %>.%
  ___
...
```{r lm2summary, warning=FALSE, record='RNP', arg='col_keys'}
___
```
...
```{r resid5, record='RNSTR', arg='labels,coordinates,scales'}
___
```
...
[] -   Le nuage de points montre une légère non linérité, peu gênante pour le modèle (A).
